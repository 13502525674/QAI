...
const instance = axios.create({
    ...
    timeout: 60000 // Increased timeout for AI generation tasks
})
...
instance.interceptors.request.use(
    function (config) {
        ...
        const isFormDataRequest = config.headers['Content-Type'] === 'multipart/form-data';
        
        if (config.method === 'post' && !isFormDataRequest) {
            // 对于普通POST请求，添加时间戳
            config.data = {
              ...config.data,
...
export function postFormData(url, formData) {
    ...
    return instance.post(url, formData, {
        headers: {
            ...
            'x_access_token': token || ''  // 确保token被添加到请求头中
        }
    })
}
