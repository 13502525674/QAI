...
public class IndexController {
    ...
    public Result getIndexData(@RequestParam("type") Integer type) {
        ...
        long count = apeUserService.count(queryWrapper);
        ...
        if (type == 0) {
            ...
            long num = apeUserService.count(wrapper);
        ...
        // 课程总数
        long courseNum = apeTaskService.count();
        jsonObject.put("courseNum", courseNum);

        // 视频总数
        long videoNum = apeChapterVideoService.count();
    ...
    public Result getLatestComments() {
        ...
        queryWrapper.lambda().orderByDesc(ApeArticleComment::getCreateTime).last("limit 20");
        List<ApeArticleComment> list = apeArticleCommentService.list(queryWrapper);
    ...
    public Result getTaskChart(@RequestParam("type")Integer type) {
        ...
        if (type == 0) {
            ...
            for (ApeTask task : taskList) {
                ...
                long count = apeTaskStudentService.count(wrapper);
                nums.add((int) count);
            }
        } else {
            QueryWrapper<ApeTask> queryWrapper = new QueryWrapper<>();
