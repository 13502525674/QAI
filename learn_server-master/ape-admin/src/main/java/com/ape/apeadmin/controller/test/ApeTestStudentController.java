...
public class ApeTestStudentController {
    ...
    public Result getTestUserState(@RequestParam("id")String id) {
        ...
        long count = apeTestStudentService.count(queryWrapper);
        if (count > 0) {
    ...
    public Result editApeTestStudent(@RequestBody JSONObject jsonObject) {
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            ...
            ApeTestStudent object = new ApeTestStudent();
            
            // 手动设置属性，避免类加载器问题
            object.setId(jsonObj.getString("id"));
            object.setItemId(jsonObj.getString("itemId"));
            object.setTestId(jsonObj.getString("testId"));
            object.setTitle(jsonObj.getString("title"));
            object.setSort(jsonObj.getInteger("sort"));
            object.setType(jsonObj.getInteger("type"));
            object.setScore(jsonObj.getInteger("score"));
            object.setKeyword(jsonObj.getString("keyword"));
            object.setAnswer(jsonObj.getString("answer"));
            object.setContent(jsonObj.getString("content"));
            object.setPoint(jsonObj.getInteger("point"));
            object.setSolution(jsonObj.getString("solution"));
            object.setUserId(jsonObj.getString("userId"));
            object.setCreateBy(jsonObj.getString("createBy"));
            object.setCreateTime(jsonObj.getDate("createTime"));
            object.setUpdateBy(jsonObj.getString("updateBy"));
            object.setUpdateTime(jsonObj.getDate("updateTime"));
            object.setAiAnalysis(jsonObj.getString("ai_analysis"));
            object.setGradedBy(jsonObj.getString("graded_by"));
